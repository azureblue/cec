...
cec::models_param cec::get_models_param(SEXP models_param_r, int n) {
    ...
    vector<shared_ptr<model_spec>> specs;
    for (int i = 0; i < len; i++) {
        ...
        switch (type) {
            ...
            case model_type::FIXED_R:
                specs.push_back(
                        make_shared<model_fixed_radius_spec>(n, params_r["r"].get<double>()));
                break;
            case model_type::COVARIANCE:
                specs.push_back(make_shared<model_covariance_spec>(n, params_r["cov"].get<mat>()));
                break;
            case model_type::EIGENVALUES:
                specs.push_back(
                        make_shared<model_eigenvalues_spec>(
                                n, params_r["eigenvalues"].get<vector<double>>())
                );
        }
    }
