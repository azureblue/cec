...
namespace cec {
    ...
    public:
        ...
        try_split_cluster(const mat &x_mat, const centers_init_spec &split_init,
                          multi_starter_params split_starts_params,
                          shared_ptr<model_spec> m_spec) {

            const starter_params &single_cluster_params = starter_params(1, split_starts_params.start_params.min_card);
            const unique_ptr<clustering_results> &single_res = cec.start(x_mat, vector<int>(x_mat.m, 0),
            ...
            unique_ptr<clustering_results> split_res = ms.start(x_mat,
                                                                vector<shared_ptr<model_spec>>({m_spec, m_spec}),
                                                                split_init,
                                                                split_starts_params);

            if (split_res && split_res->cluster_number == 2 && split_res->energy < single_cluster_energy)
                return split_res;
        ...
        unique_ptr<clustering_results>
        cec_perform_split_start(mat x_mat, const unique_ptr<clustering_results> &cl_res,
                                const centers_init_spec &split_centers_init, shared_ptr<model_spec> model_sp,
                                split_starter_params params) {

            int n = x_mat.n;
            int k = *(std::max_element(cl_res->assignment.begin(), cl_res->assignment.end())) + 1;
            ...
            for (int split_level = 0; split_level < split_depth; split_level++) {
                ...
                for (int i = 0; i < k; i++) {
                    ...
                    if (moved[i]) {
                        const unique_ptr<clustering_results> &split_res = try_split_cluster(split_x_mat,
                                                                                            split_centers_init,
                                                                                            multi_starter_params(
                                                                                                    params.params,
                                                                                                    params.split_tries,
                                                                                                    params.threads),
                                                                                            model_sp);
                ...
                if (split_flag) {
                    current_res = cec.start(x_mat, cluster, model_spec::create_models(model_sp), params.params);
        ...
        unique_ptr<clustering_results>
        cec_perform_split(mat x_mat, const unique_ptr<clustering_results> &cl_res,
                          const centers_init_spec &split_centers_init, shared_ptr<model_spec> model_sp,
                          split_starter_params params) {
            best_results_collector best;
            for (int i = 0; i < params.starts; i++)
                best(cec_perform_split_start(x_mat, cl_res, split_centers_init, model_sp, params));
